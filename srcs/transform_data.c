...
/*   Updated: 2021/09/20 16:27:14 by ysonmez          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	to_parallel(t_fdf	*fdf)
{
	...
	while (i < fdf->data.size)
	{
		...
		if (fdf->matrix[i].y == 0)
			fdf->isomatrix[i].y = fdf->matrix[i].z * sin(0.7) * fdf->screen.scale + fdf->screen.translate_y;
		else if (fdf->matrix[i].y > 0 && fdf->matrix[i].z < 0)
			fdf->isomatrix[i].y = sqrt(pow(fdf->matrix[i].y, 2) + pow(fdf->matrix[i].z, 2)) * cos(0.7 + atan(fdf->matrix[i].z / fdf->matrix[i].y) + M_PI) * fdf->screen.scale + fdf->screen.translate_y;
		else
			fdf->isomatrix[i].y = sqrt(pow(fdf->matrix[i].y, 2) + pow(fdf->matrix[i].z, 2)) * cos(0.7 + atan(fdf->matrix[i].z / fdf->matrix[i].y)) * fdf->screen.scale + fdf->screen.translate_y;
		i++;
	}
